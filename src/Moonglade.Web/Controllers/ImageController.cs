    ...
    : ControllerBase
{
    ...
    public async Task<IActionResult> Image(string filename)
    {
        ...
        {
            entry.SlidingExpiration = TimeSpan.FromMinutes(int.Parse(configuration["CacheSlidingExpirationMinutes:Image"] ?? "0"));
    ...
    public async Task<IActionResult> Image(IFormFile file, [FromQuery] bool skipWatermark = false)
    {
        ...
        {
            logger.LogError("Invalid file extension: {Ext}", ext);
            return BadRequest();
        }
        ...
        if (blogConfig.ImageSettings.IsWatermarkEnabled && !skipWatermark)
        {
            ...
            {
                logger.LogInformation("Skipped watermark for extension name: {Ext}", ext);
            }
        }
        ...
        logger.LogInformation("Image {PrimaryFileName} uploaded", primaryFileName);
